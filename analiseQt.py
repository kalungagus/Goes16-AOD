            ...
            if(date > fromDate and date < toDate) :
                ...
                if product == "AODF":
                    ...
                    if not os.path.exists('Output\\' + date.strftime("%Y\\%m\\%d\\")):
                        ...
                        print('Criado diretório Output\\' + date.strftime("%Y\\%m\\%d\\"))
                    plt.savefig('Output\\' + date.strftime("%Y\\%m\\%d\\") + 'G16_' + nomenclature + '-' + date.strftime("%Y.%m.%d.%H.%M") + '.png', dpi=DPI, bbox_inches='tight', pad_inches=0)
                    plt.close()
                    ...
                    print('GeoTIFF gerado: ', 'Output\\' + date.strftime("%Y\\%m\\%d\\") + 'G16_' + nomenclature + '_' + date.strftime("%Y-%m-%d-%H-%M") + '.tif')
                    driver.CreateCopy('Output\\' + date.strftime("%Y\\%m\\%d\\") + 'G16_' + nomenclature + '-' + date.strftime("%Y.%m.%d.%H.%M") + '.tif', grid, 0)	
                    # Close the file
                    driver = None
                    grid = None
                    # Delete the grid
